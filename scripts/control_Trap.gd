...
#
#var atk_flag = [0,0,0,0]
#var atk_time = [0,0,0,0]
func _ready():
	#player_body[0]
	pass
...
#	if player_body[0] != null:
		#trap_effect(delta)
		#print(player_body[0].get_children())
		#pass
	pass
...
#		if atk_flag[i]:
#			if atk_time[i] > 10.0:
#				player_body[i].get_child("Weapon").BULLET_DMG = player_body[i].reg_dmg
#				atk_flag[i] = 0
#				atk_time[i] = 0
#			elif atk_time[i] == 0 :
#				player_body[i].get_child("Weapon").BULLET_DMG *=2
#				atk_time[i] += delta
#			else:
#				atk_time[i] += delta
#	pass
